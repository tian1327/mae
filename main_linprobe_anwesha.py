...
def get_args_parser():
    ...
    parser.add_argument('--head_type', default='linear', choices=['linear', 'vit_head'],
                    help='Choose between a linear classifier or a ViT head for probing')
...
def main(args):
    ...
    dataset_train = MyDataset(args.dataset_root, args.train_split,
                                transform=transform_train,
                        )

    dataset_val = MyDataset(args.dataset_root, args.test_split, 
    ...
    # Add ViT Probing Head if Selected
    if args.head_type == 'vit_head':
        classifier_embed_dim = model.embed_dim
        classifier_num_heads = 12  
        classifier_depth = 12 
        model.classifier_embed = torch.nn.Linear(model.embed_dim, classifier_embed_dim, bias=True)

        model.classifier_pos_embed = torch.nn.Parameter(
            torch.zeros(1, model.patch_embed.num_patches + 1, classifier_embed_dim),
            requires_grad=False
        )
        #initialize positional embeddings
        classifier_pos_embed = get_2d_sincos_pos_embed(
            model.classifier_pos_embed.shape[-1],
            int(model.patch_embed.num_patches**0.5),
            cls_token=True
        )
        model.classifier_pos_embed.data.copy_(torch.from_numpy(classifier_pos_embed).float().unsqueeze(0))
        
        dpr = [x.item() for x in torch.linspace(0, args.drop_path, classifier_depth)]
        model.classifier_blocks = torch.nn.ModuleList([
            timm.models.vision_transformer.Block(
                classifier_embed_dim, classifier_num_heads, 4., qkv_bias=True,
                norm_layer=torch.nn.LayerNorm, drop_path=dpr[i]
            )
            for i in range(classifier_depth)
        ])
        model.classifier_norm = torch.nn.LayerNorm(classifier_embed_dim)
        model.classifier_pred = torch.nn.Linear(classifier_embed_dim, args.nb_classes)

        for name, param in model.named_parameters():
            if not name.startswith(('classifier_embed', 'classifier_blocks', 'classifier_pred', 'classifier_norm')):
                param.requires_grad = False
    
    else:  # Linear probing case
        model.head = torch.nn.Sequential(torch.nn.BatchNorm1d(model.head.in_features, affine=False, eps=1e-6), model.head)
        for name, param in model.named_parameters():
            param.requires_grad = False
        for name, param in model.head.named_parameters():
            param.requires_grad = True  # Keep only head trainable
    ...
    if args.head_type == 'vit_head':
        optimizer = torch.optim.AdamW(model_without_ddp.classifier_pred.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    else:
        optimizer = LARS(model_without_ddp.head.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    print(optimizer)
